...
from src.pipeline.weather.pipeline import pipeline 
...
def run_backfill():
    ...
    while current_start <= overall_end:
        ...
        for cat in categories:
            ...
            print(f"Ingesting: {main_cat}...")
            
            try:
                # Call your existing pipeline function
                pipeline(start_str, end_str, main_cat)
                
                # Pause for 2 seconds to be polite to the API
                time.sleep(5) 
                
            except Exception as e:
                print(f"Error on {main_cat}/for {start_str} to {end_str}: {e}")
